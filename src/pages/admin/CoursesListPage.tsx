...
import { BookOpen, Plus, X, GraduationCap, Users } from "lucide-react";
...
export function CoursesListPage() {
  ...
  const load = useCallback(async () => {
    try { setClasses(await api.listClasses(siteId)); } catch { setMsg("No se pudieron cargar los cursos."); }
  }, [siteId]);
  useEffect(() => { void load(); }, [load]);

  const filtered = useMemo(
    () => classes.filter((c) => c.name.toLowerCase().includes(search.trim().toLowerCase())),
    [classes, search],
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.length === 0 && <p className="text-muted font-semibold col-span-full py-6 text-center">No hay cursos.</p>}
        {filtered.map((c) => (
          <button
            key={c.id}
            type="button"
            onClick={() => navigate(`/admin-sede/curso/${c.id}`)}
            className="glass-card-smooth rounded-2xl p-5 text-left flex flex-col gap-3 hover:-translate-y-0.5 transition-transform cursor-pointer"
          >
